...
async def dashboard(request: Request, scan_id: int = None, db: Session = Depends(get_db)):
    ...
    if scan_id:
        ...
        selected_scan = all_scans[0] if all_scans else None
    ...
    if selected_scan:
        results = db.query(models.ScanResult).filter(models.ScanResult.scan_id == selected_scan.id).all()
...
async def generate_report(request: Request, scan_id: int = None, db: Session = Depends(get_db)):
    if scan_id:
        latest_scan = db.query(models.Scan).filter(models.Scan.id == scan_id).first()
    else:
...
async def get_topology(scan_id: int = None, db: Session = Depends(get_db)):
    if scan_id:
        selected_scan = db.query(models.Scan).filter(models.Scan.id == scan_id).first()
    else:
        selected_scan = db.query(models.Scan).order_by(models.Scan.timestamp.desc()).first()
        
    if not selected_scan or not selected_scan.results: return {"nodes": [], "edges": []}

    gw_ip = selected_scan.gateway_ip if selected_scan.gateway_ip else "192.168.1.1"
    nodes = [{"id": "gateway", "label": f"<b>Gateway</b>\n<span style='color:#64748b; font-size:10px;'>{gw_ip}</span>", "type": "gateway"}]
    edges = []

    for result in selected_scan.results:
        if result.status != "up":
            continue
        ...
        latest_historical_result = db.query(models.ScanResult).join(
            models.Scan, models.Scan.id == models.ScanResult.scan_id
        ).filter(
            models.ScanResult.ip_address == node_id,
            models.Scan.timestamp <= selected_scan.timestamp
        ).order_by(models.Scan.timestamp.desc()).first()

        has_vuln = False
        if latest_historical_result:
            for port in latest_historical_result.ports:
...
@app.get("/api/node/{ip}")
async def get_node_details(ip: str, scan_id: int = None, db: Session = Depends(get_db)):
    if scan_id:
        scan_result = db.query(models.ScanResult).filter(models.ScanResult.ip_address == ip, models.ScanResult.scan_id == scan_id).first()
    else:
        scan_result = db.query(models.ScanResult).filter(models.ScanResult.ip_address == ip).order_by(models.ScanResult.id.desc()).first()
        
    if not scan_result: return {"error": "Nodo no encontrado en este escaneo"}
...
async def host_details(ip: str, request: Request, scan_id: int = None, db: Session = Depends(get_db)):
    ...
    if not scan_result:
        return HTMLResponse(content="<h1>Host no encontrado en este escaneo.</h1>", status_code=404)
